...
// Signup with an existing username returns an error message
it('should return an error message when signing up with an existing username', async () => {
  // Mocking dependencies
  const req = {
    body: {
  ...
  // Mocking User.findOne to return an existing user with the correct username
...
it('should return an error message when signing up with an existing email', async () => {
  ...
  expect(res.status).toHaveBeenCalledWith(400); // Corrected expectation for status code
